...
ES_results <- read_tsv(here::here("results/tables/all_genes/EyeScore_ordinal_regression_results.tsv"))

# compile results.
merged_results <- ES_results %>% unique() %>%
  ...
  goi_ES = case_when(
    FDR_ES < 0.05 & FDR_inf < 0.05 & abs(est)>4~ gene_name,
    abs(est) > 2 & FDR_ES < 0.05 & FDR_inf < 0.05 & FDR_diet < 0.05 & FDR_ixd < 0.05 ~ gene_name,
    TRUE ~ NA
  )
  )  %>% unique()
```
...
```{r}
# Create Functions
run_gseGO_analysis <- function(top_tags, exponent = 0.8, ont = "BP") {
  gene_list <- top_tags %>%
    arrange(desc(logFC)) %>%
    pull(logFC, gene)
...
run_gseKEGG_analysis <- function(top_tags, exponent = 0.8, organism = 'scan') {
  gene_list <- top_tags %>%
    arrange(desc(logFC)) %>%
    pull(logFC, gene)
...
```{r}
#gseGO
gseGO_ixd <- run_gseGO_analysis(topTags_ixd_edgeR)
  topTags_ixd_edgeR %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseGO(
...
#                                     ),
#              multiple="first",
#             suffix = c("_ES", "_inf")) %>% unique()

# write_tsv(ES_results, here::here("results/tables/all_genes/EyeScore_ordinal_regression_results.tsv"))
...
```{r}
# enrichKEGG_ES_ord <- enrichKEGG(gene = ES_results %>%
#   filter(FDR_ES < 0.05) %>%
#   #filter(`Pr(>|z|)` < 0.0005) %>%
#   pull(entrezgene_id)  ,
#                organism = 'scan', 
...
# enrichGO_ES_ord_up <- ES_results %>%
#   filter(FDR_ES < 0.05 & Estimate > 0) %>%
#   #filter(`Pr(>|z|)` < 0.0005) %>%
#   pull(entrezgene_id) %>%
#   enrichGO(     gene          = .,
#                 OrgDb         = orgdb,
...
# 
# enrichGO_ES_ord_down <- ES_results %>%
#   filter(FDR_ES < 0.05 & Estimate < 0) %>%
#   #filter(`Pr(>|z|)` < 0.0005) %>%
#   pull(entrezgene_id) %>%
#   enrichGO(     gene          = .,
...
phyper(q, m, n_genes- m, k, lower.tail=FALSE)
...
heatmap_gene_clusters <- tibble(
  ...
  id = unlist(lapply(row_order(ht), function(x) rownames(logFC_matrix)[x]))
                ) |> separate_wider_delim(cols = id, names = c("gene_label", "description"), delim = " - ") |> 
  left_join(df_heatmap, by = c("gene_label" = "nice_gene_name", "description" = "description")) %>%
...
# enrichKEGG_ES_ord@result$Description <- gsub(enrichKEGG_ES_ord@result$Description, pattern = " \\- Serinus canaria \\(common canary\\)" , replacement = "")

# network_ES_plot <- enrichGO_ES_ord 
# (enrichKEGG_ES_ord+ enrichGO_ES_ord) #%>%
# setReadable(orgdb) %>%
...
#            color.params = list(foldChange = ES_results %>%
...
```{r}
# all_de_genes = list(
#   "Eye Score" = merged_results %>% filter(FDR_ES < 0.05) %>% dplyr::select(entrezgene_id.x) %>% drop_na() %>% pull(entrezgene_id.x) %>% as.character(),
...
ggsave(plot = g, filename= here("results/figures/fig4b_network_annotated.svg"), 
...
ggsave(plot = g_unannotated, filename= here("results/figures/fig4b_network.svg"), 
...
```{r}
fig4B_new <- ggdraw() + draw_image(here("results/figures/fig4b_new_network_plot.png")) + theme_void()
```
...
fig.s3 <- merged_results %>%
  dplyr::select(DE_inf, gene.z,entrezgene_id.x, Estimate, logFC_inf, logFC_inf_lipid, logFC_inf_prot,
  ...
  dplyr::select(-DE_inf) %>%
  mutate(rowid = paste0(gene.z, "_", entrezgene_id.x)) %>%
  group_by(rowid) %>%
  slice_head(n=1) %>%
  column_to_rownames("gene.z") %>%
  dplyr::select(-c(rowid, entrezgene_id.x)) %>%
...
nudge_x = -0.4,force=0.1,
        ...
        'Infection\n(lipid)'=(
            ggplot(mapping=aes(y=logFC_inf_lipid,color=-log10(FDR_inf_lipid), label=goi)) +
              ...
              ggbeeswarm::geom_quasirandom(alpha=0.5, size=pt_size,
                na.rm=TRUE)+ ggrepel::geom_text_repel(
                                                       direction = "y",size=lab_size,nudge_x = -0.4, force=0.1, max.overlaps = 7) + scale_color_continuous(name="-log10(FDR) ") + theme(legend.position='none')
        ),
        'Infection\n(protein)'=(
            ggplot(mapping=aes(y=logFC_inf_prot,color=-log10(FDR_inf_prot), label=goi)) +
              ...
              ggbeeswarm::geom_quasirandom(alpha=0.5, size=pt_size,
                na.rm=TRUE) + 
              ggrepel::geom_text_repel(direction = "y",size=lab_size,nudge_x = -0.4, force=0.1, max.overlaps = 7) + 
        ...
        'Eye Score'=(
            ggplot(mapping=aes(y=est,color=-log10(FDR_ES),label=goi)) +
              geom_hline(yintercept=0, linetype="solid", color = "grey30") +
...
ggsave(plot = fig.s5, filename = here("results/figures/fig.s5.png"),        width = 6, height = 6, units = "in", dpi=1200, bg="white")
```
...
#ts3 = mk_par(ts3, j=4:5, 
#             value = as_paragraph(fmt_signif_after_zeros(c(`Set Size`, `Enrich Score`  ),digits=2)))
...
read_docx() %>%
  ...
  print(target = here("results/tables/supplemental_table_S2.docx"))
# doc <- body_add_flextable(doc, ts2,pos = "after")

# print(doc, target = here("results/tables/supplemental_tables.docx"))

# read_docx() %>%
#   body_add_flextable(value = ts2) |>
#   
#   print(target = here("results/tables/supplemental_tables.docx"))
```
...
ts3 = gseGO_all_df |>
  ...
  mutate(across(is.numeric, signif, 3)) |> 
  dplyr::select(comparison, ID, Description, setSize, enrichmentScore,  p.adjust ) |>
...
read_docx() %>%
  ...
  print(target = here("results/tables/supplemental_table_S3.docx"))
