...
# Prepare the list of gene identifiers
gene_list_diet_up <- uninfected_both_topTags %>%
  dplyr::filter(logFC > 0.2) %>%
  pull(gene)

gene_list_diet_down <- uninfected_both_topTags %>%
  dplyr::filter(logFC < -0.2) %>%
  pull(gene)
...
enrichplot::upsetplot(dream_diet_kegga_up, title = "Protein Diet Upregulated KEGG Pathways")
...
ordered_list_diet <- uninfected_both_topTags %>%
  # significant_diet_genes %>%
...
enrichplot::upsetplot(dream_diet_gseKEGG)

enrichplot::gseaplot2(dream_diet_gseKEGG, geneSetID = c(1,3), title = dream_diet_gseKEGG$Description[1])
```

```{r}
scan00100 <- pathview::pathview(gene.data  = diet_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene),
                     pathway.id = "scan00100",
                     species    = "scan"#,
  #                    limit      = list(gene=max(abs(uninfected_both_topTags  %>%
  # arrange(desc(logFC)) %>%
  # pull(logFC, gene))), cpd=1)
  )

KEGG_cytokine <- pathview::pathview(gene.data  = infVSuninf_both_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene),
                     pathway.id = "scan04060",
                     species    = "scan"#,
  #                    limit      = list(gene=max(abs(uninfected_both_topTags  %>%
  # arrange(desc(logFC)) %>%
  # pull(logFC, gene))), cpd=1)
...
# Diet effects in infected vs uninfected samples: overlap analysis
```{r}
# Rank genes based on logFC, pValue, or a combination (here we use absolute logFC)
df_infected_ranked <- infected_protVSlipid_topTags %>%
  filter(P.Value<0.05) %>%
  arrange(desc(abs(logFC)))

df_uninfected_ranked <- uninfected_both_topTags %>%
  filter(P.Value<0.05) %>%
  arrange(desc(abs(logFC)))

# Identify the top 1000 genes in both conditions
# Overlap analysis
common_genes <- intersect(df_infected_ranked$gene, df_uninfected_ranked$gene)
```

```{r}
# Statistical analysis of overlap using Fisher's Exact Test
# Create a contingency table
total_genes <- length(unique(c(infected_protVSlipid_topTags$gene, uninfected_both_topTags$gene)))
top_infected_not_uninfected <- length(setdiff(df_infected_ranked$gene, df_uninfected_ranked$gene))
top_uninfected_not_infected <- length(setdiff(df_uninfected_ranked$gene, df_infected_ranked$gene))
not_top_in_both <- total_genes - length(unique(c(df_infected_ranked$gene, df_uninfected_ranked$gene)))

contingency_table <- matrix(c(length(common_genes), top_infected_not_uninfected,
                              top_uninfected_not_infected, not_top_in_both),
                            nrow = 2, byrow = TRUE)
colnames(contingency_table) <- c("Top in Both", "Top in One Only")
rownames(contingency_table) <- c("Top in Infected", "Not Top in Infected")

fisher_result <- fisher.test(contingency_table)

# Correlation analysis for common genes
# Subset the data frames to only include common genes
df_infected_common <- df_infected_ranked %>% filter(gene %in% common_genes)
df_uninfected_common <- df_uninfected_ranked %>% filter(gene %in% common_genes)

# Ensure the genes are in the same order for correlation analysis
df_infected_common <- df_infected_common %>% arrange(gene)
df_uninfected_common <- df_uninfected_common %>% arrange(gene)

# Calculate Spearman correlation
correlation_result <- cor.test(df_infected_common$logFC, df_uninfected_common$logFC, method = "spearman")

# Output the results
print(paste("Number of common genes:", length(common_genes)))
print(paste("Fisher's Exact Test p-value:", fisher_result$p.value))
print(paste("Spearman Correlation between common genes' logFC:", correlation_result$estimate))
```

```{r}
dream_diet_across <- enrichKEGG(gene = common_genes,
                 organism = 'scan',
                 universe = diet_topTags$gene
                 #pvalueCutoff = 0.05
                 )

dream_diet_across@result %>% View()
```
...
y$counts %>%
  ...
  #filter(day==0) %>%
  ggplot(aes(x = diet, y = counts, color = condition, group = "BirdID")) +
    geom_point(position = "jitter") +
    # geom_line() +
    # facet_wrap(~diff_day) +
    theme_bw()
...
below is just for fun, not a good viz
```{r}
# swapping which is column and which is color.
y$counts %>%
  as.data.frame() %>%
  rownames_to_column("gene") %>%
  pivot_longer(-gene, names_to = "sample", values_to = "counts") %>%
  ...
  #filter(diff_day == "diff_14") %>%
  distinct() %>% #View()
  ggplot(aes(x=counts_diff_norm, color = diet, fill = diet
             )) +
  geom_density(alpha=0.5) +
  xlim(-0.0005,0.0005) +
  facet_grid(rows=vars(diff_day), cols=vars(condition)) +
  # scale_y_log10() +
  theme_minimal()
```
...
Tolerance: get the regression. Get the individual level tolerance. Correlate the uninfected day0 fold changes with index. How do conditions of host on day 0 impact later tolerance after infection by diet. What Erin wants to know is: LogFC is predicting the future response. Tolerance. Question is how does expression prior to infection predict tolerance after infection. We could do day 7.      tl;dr: let’s see how expression on day 0 correlates with (ES/log10load) onTl;dr7. or day 14, or merge them all.
...
contrast_ixn_tolerance <- makeContrasts(#design, #metadata, 
  contrasts = c("(diet_toleranceprotein.3 - diet_toleranceprotein.1) - (diet_tolerancelipid.3 - diet_tolerancelipid.1)",
                # "Diet_1protein",
                "(diet_toleranceprotein.3 + diet_toleranceprotein.2 + diet_toleranceprotein.1)/3-(diet_tolerancelipid.3 + diet_tolerancelipid.2 + diet_tolerancelipid.1)/3",
                "(diet_toleranceprotein.3 + diet_tolerancelipid.3)/2-(diet_toleranceprotein.1 + diet_tolerancelipid.1)/2"
                #"diet_tolerancelipid.3 - diet_tolerancelipid.1",
                #"diet_toleranceprotein.3 - diet_tolerancelipid.3"
                ), levels=design)
...
res1 <-  glmQLFTest(fit, contrast=contrast_ixn_tolerance[,1])

# res1 <-  glmQLFTest(fit, coef = "Diet_1protein:tolerance_1")
#res2 <-  glmQLFTest(fit, coef = "Diet_1lipid:tolerance_1")
# 
topTags_tolerance_ixn <- topTags(res1, n=Inf) %>% data.frame() %>%
  arrange(FDR) %>%
...
# are those genes enriched for anything? Yes they are.
...
## get list of genes that are shared between diet:infection
```{r}
# up in infected and protein
intersect(
  infVSuninf_both_topTags  %>%
  arrange(desc(logFC)) %>% #View()
  head(2000) %>%# View()
  pull(gene),
  uninfected_both_topTags %>%
  arrange(desc(logFC)) %>%
...
# correlate whole gene list:
infVSuninf_both_topTags  %>%
  arrange(logFC) %>%
  mutate(contrast = "infection") %>%
  left_join(by = c("gene", "external_gene_name"), uninfected_both_topTags) %>%# View()
  mutate(DE_proteinUninfected_GO = ifelse(gene %in% intersect(
  # higher in uninfected
  infVSuninf_both_topTags %>%
    #filter(P.Value < 0.1) %>%
  arrange(logFC) %>% #View()
  head(2000) %>%# View()
  ...
  mutate(DE_proteinUninfected_GO = ifelse(gene %in% c("103821617","103812985","103812987","103813169","103818543","103815545
"),"green", DE_proteinUninfected_GO)) %>%
  # filter(DE_proteinUninfected_GO != "red") %>%
  ggplot(.,aes(x=logFC.x, y=logFC.y,color=DE_proteinUninfected_GO)) +
  geom_point(alpha=.2) +
  theme_bw()
```
...
Here, we see there exists a significant overlap between the genes that are higher in protein birds and those that are repressed when infected. The GO terms enriched in this overlap are 

```{r}
unfiltered_genes_with_GO <- HDO.db::select(orgdb, keys(orgdb), "GO", "ENTREZID") %>%
  data.frame() %>%
  filter(!is.na(GO)) %>%
  filter(ENTREZID %in% diet_topTags$gene) %>%
  pull(ENTREZID) %>%
  unique()

unfiltered_genes_with_KEGG <- HDO.db::select(orgdb, keys(orgdb), "KEGG", "ENTREZID") %>%
  data.frame() %>%
  filter(!is.na(KEGG)) %>%
  filter(ENTREZID %in% diet_topTags$gene) %>%
  pull(ENTREZID) %>%
...
### For infection vs uninfection

There are not many genes with annotations when using strict cutoffs, using these cutoffs gets a reasonable number of genes on which to run enrichment.
```{r}
GO_higher_in_inf  <- enrichGO(gene         = 
  infVSuninf_both_topTags  %>%
  # filter(gene %in% unfiltered_genes_with_GO) %>%
  # filter(P.Value < 0.5) %>%
  filter(logFC > 0.2) %>%
...
```{r}
GO_higher_in_prot  <- enrichGO(gene         = 
  uninfected_both_topTags  %>%
  # filter(gene %in% unfiltered_genes_with_GO) %>%
  # filter(P.Value < 0.5) %>%
  filter(logFC > 0.2) %>%
...
GO_higher_in_lipid  <- enrichGO(gene         = 
  uninfected_both_topTags  %>%
  # filter(adj.P.Val < 0.2) %>%
  filter(logFC < -0.2) %>%
  filter(gene %in% unfiltered_genes_with_GO) %>%
...
```{r}
#?? expect to see enrichments
GO_higher_in_protein_diet_and_uninf  <- enrichGO(gene         =  intersect(
  # higher in uninfected
  infVSuninf_both_topTags  %>%
    #filter(P.Value < 0.1) %>%
  arrange(logFC) %>% #View()
...
#
GO_higher_in_protein_diet_and_inf  <- enrichGO(gene         =  intersect(
  infVSuninf_both_topTags  %>%
  # higher in infected
  arrange(desc(logFC)) %>% #View()
  tail(2000) %>%# View()
...
# No enrichments in genes up in non-infection and protein diet.
GO_higher_in_lipid_diet_and_uninf  <- enrichGO(gene         =  intersect(
  infVSuninf_both_topTags  %>%
  # higher in uninfected
    #filter(P.Value < 0.1) %>%
  arrange(logFC) %>% #View()
...
# No enrichments in genes higher in both lipid and infection
GO_higher_in_lipid_diet_and_inf  <- enrichGO(gene         =  intersect(
  infVSuninf_both_topTags  %>%
  # higher in uninfected
    #filter(P.Value < 0.1) %>%
  arrange(desc(logFC)) %>% #View()
...
# visualize GO relationships
```{r}
# protein + infection
# no significant enrichments
# ggplot(GO_higher_in_protein_diet_and_inf,
#        showCategory = 15,
...
#   xlab("Rich Factor") +
#   ylab(NULL) +
#   ggtitle("Enriched GO terms in Infection and Protein Diet") +
#   theme_bw()

# lipid + non-infection
ggplot(GO_higher_in_lipid_diet_and_uninf,
       showCategory = 10,
       aes(richFactor, fct_reorder(Description, richFactor))) +
  geom_segment(aes(xend = 0, yend = Description)) +
  geom_point(aes(color = p.adjust, size = Count)) +
  ...
  xlab("Rich Factor") +
  ylab(NULL) +
  ggtitle("Enriched GO terms in non-infection and lipid Diet") +
  theme_bw()


# protein + non-infection
ggplot(GO_higher_in_protein_diet_and_uninf,
       showCategory = 10,
       aes(richFactor, fct_reorder(Description, richFactor))) +
  geom_segment(aes(xend = 0, yend = Description)) +
  geom_point(aes(color = p.adjust, size = Count)) +
  ...
  xlab("Rich Factor") +
  ylab(NULL) +
  ggtitle("Enriched GO terms in non-infection and Protein Diet") +
  theme_bw()

# lipid + infection
# ggplot(GO_higher_in_lipid_diet_and_inf,
#        showCategory = 15,
#        aes(richFactor, fct_reorder(Description, richFactor))) +
#   geom_segment(aes(xend = 0, yend = Description)) +
#   geom_point(aes(color = p.adjust, size = Count)) +
...
#   xlab("Rich Factor") +
#   ylab(NULL) +
#   ggtitle("Enriched GO terms in infection and lipid Diet") +
#   theme_bw()


#### need for paper..
# gseKEGG for infection logFC values
gseKEGG_infection <- infVSuninf_both_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_infection)

# gseKEGG for infection logFC values
# no enrichments fount
gseKEGG_prot_infection <- infected_protein_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_prot_infection)

# gseKEGG for infection logFC values
gseKEGG_lipid_infection <- infected_lipid_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_lipid_infection)


# gseKEGG for diet logFC values
gseKEGG_diet <- uninfected_both_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_diet, title = "diet gseKEGG")


gseKEGG_diet_all <- diet_topTags %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_diet_all, title = "diet ALL gseKEGG")



enrichplot::upsetplot(enrichGO(gene         = 
  infVSuninf_both_topTags  %>%
  # filter(gene %in% unfiltered_genes_with_GO) %>%
  # filter(P.Value < 0.5) %>%
  filter(logFC < -0.2) %>%
...
# Continue tolerance analysis here
```{r}
#makeContrasts()
...
topTags_tolerance3v1 <- topTags(res_tolerance3v1, n=Inf) %>% data.frame() %>% 
  arrange(FDR) %>%
  # mutate(logFC=round(logFC,4)) %>%
  # mutate(across(where(is.numeric), signif, 3)) %>%
  # mutate_if(is.numeric, signif, 3) %>%
  # remove_rownames() |> 
  rownames_to_column("gene") %>% 
...
gseKEGG_tolerance3v1 <- topTags_tolerance3v1  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_tolerance3v1, title = "tolerance3v1 gseKEGG")

topTags_tolerance_diet <- topTags(res_diet_prot_v_lipid, n=Inf) %>% data.frame() %>% 
  arrange(FDR) %>%
  # mutate(logFC=round(logFC,2)) %>%
  # mutate(across(where(is.numeric), signif, 3)) %>%
  # mutate_if(is.numeric, signif, 3) %>%
...
gseKEGG_tolerance_diet <- topTags_tolerance_diet  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_tolerance_diet, title = "tolerance diet gseKEGG")

enrichKEGG_tolerance_lipid_DE <- enrichKEGG(gene = filter(topTags_tolerance_diet, logFC < 0 & FDR < 0.05) |> pull(gene), organism = 'scan', pvalueCutoff = 0.05)

# included in paper, sees steroid metabolism
enrichKEGG_tolerance_prot_DE <- enrichKEGG(gene = filter(topTags_tolerance_diet, logFC > 0.5 & FDR < 0.05) |> pull(gene), organism = 'scan', pvalueCutoff = 0.05)

# do same but GO terms
# gseGO_tolerance_diet <- topTags_tolerance_diet  %>%
#   arrange(desc(logFC)) %>%
#   pull(logFC, gene) %>%
#   gseGO(geneList     = .,
#                OrgDb = orgdb,
#                #universe = topTags_tolerance_diet$gene,
#                minGSSize    = 10,
#                maxGSSize = 35,
#                pvalueCutoff = 0.5,
#                verbose      = FALSE)
# 
# enrichplot::upsetplot(gseGO_tolerance_diet, title = "tolerance diet gseGo")


enrichGO_tolerance_prot_DE  <- enrichGO(gene         = 
...
# lipid GO
enrichGO_tolerance_lipid_DE  <- enrichGO(gene         = 
  topTags_tolerance_diet  %>%
  filter(FDR < 0.05) %>%
  filter(logFC < 0) %>%
  pull(gene)
...
enrichKEGG_tolerance_lipid <- enrichKEGG(gene = filter(topTags_tolerance_diet, logFC < -0.2) |> pull(gene), organism = 'scan', pvalueCutoff = 0.2)
enrichKEGG_tolerance_prot <- enrichKEGG(gene = filter(topTags_tolerance_diet, logFC > 0.2) |> pull(gene), organism = 'scan', pvalueCutoff = 0.2)
...
# these could be useful in the paper. Specifically, KEGG categories enriched in extremes of interaction between diet and tolerance.
enrichKEGG_tolerance_ixn_lipid <- enrichKEGG(gene = filter(top_tolerance_genes, logFC < -0.2) |> pull(gene), organism = 'scan', pvalueCutoff = 0.5)

enrichKEGG_tolerance_ixn_prot <- enrichKEGG(gene = filter(top_tolerance_genes, logFC > 0.2) |> pull(gene), organism = 'scan', pvalueCutoff = 0.5)


gseKEGG_tolerance_ixn <- top_tolerance_genes  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene) %>%
  gseKEGG(geneList     = .,
               organism = 'scan',
...
enrichplot::upsetplot(gseKEGG_tolerance_ixn, title = "tolerance interaction gseKEGG")
...
# plotting individual genes (tolerance)
```{r}
gene_diff_count_df <- y_tolerance$counts %>%
  as.data.frame() %>%
  rownames_to_column("gene") %>%
  pivot_longer(-gene, names_to = "sample", values_to = "counts") %>%
  separate_wider_delim(sample, names = c("sampleID","BirdID", "condition-diet", "day"), delim = "_", too_many = "merge") %>% #View()
  separate_wider_delim(`condition-diet`, names = c("condition","diet"), delim = ".", too_many = "merge") %>%
  # filter(gene == "103826733") %>% #CPT1A, top DE gene for diet
  # filter(gene == "103822409") %>% #FGD2, gene that is expressed in macrophages
  # filter(gene == "103823426") %>%
...
gene_diff_count_df %>%
  filter(diff_day == "diff_14") %>%
  ggplot(aes(x=counts_diff, color = interaction(condition,diet), fill = interaction(condition,diet)
             )) +
  geom_density(alpha=0.5) +
  xlim(-150,150) +
  facet_wrap(~diet) +
...
gene_data_logFC_diet_tolerance_prot <- res_diet_prot_v_lipid$table %>% dplyr::select(logFC)

# put the ORF names as names for each entry
fold_change_geneList_diet_tolerance_prot <- setNames(object = data.frame(res_diet_prot_v_lipid)$logFC,
                                 nm = rownames(res_diet_prot_v_lipid))
...
head(fold_change_geneList_diet_tolerance_prot)

# genes higher in lipid diet, just considering diet.
# no difference found..
DE_tol_genes_diet_tolerance_lipid <-  res_diet_prot_v_lipid %>% 
  data.frame() %>%
  filter(PValue < 0.01 & (logFC) < -1) %>%
  rownames_to_column() |>
  pull(rowname)

diet_tolerance_lipid_kegg_results <- enrichKEGG(gene = DE_tol_genes_diet_tolerance_lipid, #entrez_ids$ENTREZID, 
                           organism = 'scan' #options: https://www.genome.jp/kegg/catalog/org_list.html
                           )

# genes higher in protein diet
DE_tol_genes_diet_tolerance_prot <-  res_diet_prot_v_lipid %>% 
  data.frame() %>%
  filter(PValue < 0.01 & logFC > 1) %>%
  rownames_to_column() |>
  pull(rowname)

diet_tolerance_prot_kegg_results <- enrichKEGG(gene = DE_tol_genes_diet_tolerance_prot, #entrez_ids$ENTREZID, 
                           organism = 'scan' #options: https://www.genome.jp/kegg/catalog/org_list.html
                           )

diet_tolerance_prot_kegg_results@result$Description <- diet_tolerance_prot_kegg_results@result$Description %>% print() %>% str_replace_all(., fixed(" - Serinus canaria"), "")

```
### visualize those results
```{r}
enrichplot::upsetplot(diet_tolerance_prot_kegg_results)
...
# omit any NA values 
kegg_gene_list = na.omit(fold_change_geneList_diet_tolerance_prot)
...
browseKEGG(gse_kegg, 'scan00100')

scan00100 <- pathview::pathview(gene.data  = kegg_gene_list,
                     pathway.id = "scan00100",
                     species    = "scan",
                     limit      = list(gene=max(abs(kegg_gene_list)), cpd=1))

scan00100 <- pathview::pathview(gene.data  = uninfected_both_topTags  %>%
  arrange(desc(logFC)) %>%
  pull(logFC, gene),
                     pathway.id = "scan00100",
                     species    = "scan"#,
...
mergedl2fc <- top_tolerance_genes |>
  left_join(topTags_tolerance_diet, by = c("gene", "external_gene_name", "entrezgene_description"), suffix=c("_tolerance_score_infection","_tolerance_score_diet")) |>
  left_join(topTags_tolerance_ixn, by = c("gene", "external_gene_name", "entrezgene_description")) |>
  left_join(infVSuninf_both_topTags |> mutate(gene = str_replace(gene, "^LOC", "")), by = c("gene", "external_gene_name", "entrezgene_description"), suffix=c("_tolerance_score_interaction","_dream_MG")) |>
  left_join(uninfected_both_topTags |> mutate(gene = str_replace(gene, "^LOC", ""), logFC_dream_Diet = logFC) |> select(-logFC), by = c("gene", "external_gene_name", "entrezgene_description"), suffix=c("_dream_MG","_dream_Diet"))

col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))

mergedl2fc |>
  select(starts_with("logFC")) |>
  cor(use = "pairwise.complete.obs", method = "pearson") |> #View()
...
clean_and_mutate_data <- function(data, sig.cutoff=0.05, logFC.cutoff=0.575) {
  data %>%
    ...
    mutate(delabel = case_when(
      adj.P.Val < sig.cutoff ~ delabel,
      abs(logFC) > 0.575 ~ delabel,
      TRUE ~ NA
    )) %>%
    mutate(diffexpressed = case_when(
      adj.P.Val > sig.cutoff ~ "NO",
      logFC < 0 ~ "DOWN",
      logFC > 0 ~ "UP"
    )) %>%
...
create_gene_plot <- function(data, title, sig.cutoff=0.05, logFC.cutoff=0.575) {
...
ggsave(here("infVSuninf_both_topTags.png"), width = 15, height = 10, units = "cm", dpi=300)
