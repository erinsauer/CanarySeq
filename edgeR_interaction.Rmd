...
metadata_phenotypes <- metadata %>%
  ...
  # mutate(tolerance = (logLoad/(ES_day0_analysis+1))-1) # current tolerance index
  # mutate(tolerance = log((logLoad)/(ES_day0_analysis+1))) # new better tolerance index
  # mutate(tolerance = (logLoad)-log(ES_day0_analysis+1)) %>% # is this even better??
  mutate(tolerance = (1+logLoad)/(ES_day0_analysis+1)-1) # this is the best one??
...
# cpm_cutoff <- 1
# apply cutoff
keep <- rowSums(cpm(y_ixn) > cpm_cutoff) >= 8
y_ixn <- y_ixn[keep,]
summary(keep)
...
# # cpm_cutoff <- 1
# # apply cutoff
# keep_tolerance <- rowSums(cpm(y_tolerance) > cpm_cutoff_tolerance) >= 8
...
# above design works well, but hard to make contrasts. use pseudogroupings instead
# generate groupings
tolerance <- metadata_phenotypes_ixn$tolerance_median #%>% as.factor()
...
# cpm_cutoff <- 1
# apply cutoff
keep_ES <- rowSums(cpm(y_ES) > cpm_cutoff_ES) >= 8
y_ES <- y_ES[keep_ES,]
summary(keep_ES)
...
## logLoad
```{r}
logLoad <- metadata_phenotypes_ixn$logLoad
...
# cpm_cutoff <- 1
# apply cutoff
keep_logLoad <- rowSums(cpm(y_logLoad) > cpm_cutoff_logLoad) >= 8
...
genes_higher_protein_diet <- topTags_diet_edgeR %>%
  filter(FDR < 0.05 & logFC > 0) %>%
  dplyr::select(gene, logFC) %>%
  deframe() 

genes_higher_lipid_diet <- topTags_diet_edgeR %>%
  filter(FDR < 0.05 & logFC < 0) %>%
  dplyr::select(gene, logFC) %>% 
  deframe() 
...
DE_genes_edgeR_upset %>%
  upset(., colnames(DE_genes_edgeR_upset[,c(21:23)]), name= "Comparison", width_ratio = 0.2,
        #mode="union",
        #intersections="all",
        annotations = list(
        # 1st method - passing list:
...
ggsave("figures/DE_genes_edgeR_combined_upsetplot.png", width = 8, height = 8, units = "in", dpi = 300)
```
Venn diagram
```{r}
...
```{r}
genes_DE_diet <- topTags_diet_edgeR %>%
  filter(FDR < 0.05) %>%
  pull(gene) %>%
  unique()
...
df_heatmap_diet_tpm <- tpm_df %>%
  filter(entrezID %in% c(genes_DE_diet)) %>%
  # filter(Day %in% c(0,14)) %>%
  mutate(nice_gene_name = case_when(
    !is.na(gene_name) ~ gene_name,
    !is.na(gene) ~ paste0("LOC",gene),
...
tpm_data %>%
  ...
  distinct() %>%# View()
  ggplot(aes(x=counts_diff_norm,  color = condition, fill = condition
             )) +
...
logFC_all_contrasts <- topTags_inf_edgeR %>%
  ...
  mutate(repressed_in_infection = gene %in% names(genes_lower_in_infection),
        ...
        logFC_diet = topTags_diet_edgeR |> arrange(gene) |> filter(gene == gene) |> pull(logFC),
        #case_when(
          # gene %in% names(genes_higher_protein_diet) ~ genes_higher_protein_diet[gene],
          # gene %in% names(genes_higher_lipid_diet) ~ genes_higher_lipid_diet[gene],
          # TRUE ~ NA_real_
  ...
  #case_when(
        ...
        FDR_infection = FDR, #case_when(
          ...
          # TRUE ~ NA_real_
       # ),
        FDR_diet = topTags_diet_edgeR |> arrange(gene) |> filter(gene == gene) |> pull(FDR),
  # case_when(
  #         gene %in% names(fdr_genes_diet) ~ fdr_genes_diet[gene],
  #         # gene %in% names(genes_higher_protein_diet) ~ filter(topTags_diet_edgeR, paste0(gene) == paste0(gene))$FDR,
  #         # gene %in% names(genes_higher_lipid_diet) ~ filter(topTags_diet_edgeR, paste0(gene) == paste0(gene))$FDR,
...
logFC_matrix <- as.matrix(df_heatmap[, c("logFC_infection", "logFC_diet", "logFC_interaction")])
rownames(logFC_matrix) <-  paste0(df_heatmap$nice_gene_name, " - ", df_heatmap$description)
FDR_matrix <- as.matrix(df_heatmap[, c("FDR_infection", "FDR_diet", "FDR_interaction")])
...
colnames(logFC_matrix) <- c("Infection", "Diet", "Interaction")
...
heatmap <- Heatmap(logFC_matrix, name = "logFC", 
                   ...
                   cell_fun = function(j, i, x, y, width, height, fill) {
                       ...
                       if (significance_matrix[i, j] == "*") {
                           grid.text("X", x, y, gp = gpar(col = "black", fontsize = 6))
                       }
                   })
...
png(file="figures/logFC_heatmap.png", width=6, height=9, res=1200,units = "in")
...
PCA <- plotMDS(y_ixn, gene.selection="common",
               top = 2000,pch = 2, cex = 0.5, main = "PCA plot of gene expression for all samples", dim.plot=c(1,2))
```
...
edgeR_analyzed_genes <- topTags_inf_edgeR %>% left_join(result_BM_merged, by=c("gene" = "entrezgene_id")) %>% #View()
...
n <- length(unique(edgeR_analyzed_genes))#1000
...
tbl_2 <- tbl %>% mutate(all = map(formulas, mod_f) %>% map(summary) %>% map(coef) %>% map(as_tibble, rownames = "gene")
                        # gla = all %>% map(summary),
...
tbl_2$all %>% set_names(rownames(tpm_data)[1:n]) %>% dplyr::bind_rows() %>%
  filter(!str_detect(gene, "Intercept")) %>% View()# left_join(result_BM_merged, by=c("gene" = "gene_tag")) %>% View()
...
#Create Final Fig 4 comparing GO term enrichments.
```{r, fig.height=8}
fig4_plot_list <- list(
...
DE_GO_terms_any_inf <- bind_rows(fig4_plot_list[[1]], 
          fig4_plot_list[[2]],
          fig4_plot_list[[3]]) %>%
...
bind_rows(fig4_plot_list[[1]], 
          fig4_plot_list[[2]],
          fig4_plot_list[[3]]) %>%
  filter(ID %in% DE_GO_terms_any_inf) %>%
...
ggsave("figures/GO_term_enrichment_comparison.png", width = 12, height = 8, units = "in", dpi = 600)
```
...
bind_rows(fig4_plot_list[[1]], 
  ...
  left_join(topTags_inf_edgeR, by=c("core_enrichment"="gene")) %>%
  left_join(topTags_protein_inf_edgeR, by=c("core_enrichment"="gene"), suffix = c("all", "protein")) %>%
  left_join(topTags_lipid_inf_edgeR, by=c("core_enrichment"="gene")) %>%
  left_join(topTags_diet_edgeR, by=c("core_enrichment"="gene"), suffix = c("lipid", "ES")) %>%
  # pivot_longer(cols = c(logFCall, logFCprotein, logFClipid, logFCES), names_to = "contrast_logFC", values_to = "logFC") %>%
  # pivot_longer(cols = c(FDRall, FDRprotein, FDRlipid, FDRES), names_to = "contrast_FDR", values_to = "FDR") %>%
  # filter(contrast_logFC != "logFCES") %>%
  mutate(logFC_interesting = case_when(
    comparison == "inf_all" ~ logFCall,
    comparison == "inf_protein" ~ logFCprotein,
    comparison == "inf_lipid" ~ logFClipid
  )) %>%
  mutate(big_diff_label = case_when(logFC_interesting > 1 ~ gene_nameall,
                                    logFC_interesting < -1 ~ gene_nameall,
                                    TRUE ~ NA)) %>% View()
  ggplot(aes(x= Description, y = logFC_interesting, color = comparison, fill = comparison, #alpha=-log10(FDR),
             label=big_diff_label)) + 
  geom_point() +
  ggrepel::geom_text_repel(aes(label = big_diff_label)) +
  coord_flip() +
  facet_wrap(~comparison)
```
...
```{r}
genes_to_plot <- c("103812987", # chemokine c-c ixn diff bigger in protein tolerance
...
y_ixn$counts %>%
  ...
  #filter(day==0) %>%
  #drop_na() %>% View()
  ggplot(aes(x = diet, y = counts_diff, color = condition, group = "BirdID", size=tolerance_cat_num)) +
...
y_ixn$counts %>%
  ...
  pivot_longer(c("day14", "day21", "day0"), names_to = "day", values_to = "counts") %>% #View()
  #filter(day==0) %>%
  ggplot(aes(x = diet, y = counts, color = condition, shape=day, group = "BirdID", size=tolerance_cat_num)) +
    # geom_point(position = "jitter") +
    ggbeeswarm::geom_quasirandom(alpha=1) +
    # geom_line() +
    # facet_wrap(~day) +
...
y_tolerance$counts %>%
  ...
  pivot_longer(c("day14", "day21", "day0"), names_to = "day", values_to = "counts") %>% #View()
  #filter(day==0) %>%
  ggplot(aes(x = tolerance_median, y = (counts)#, color = condition#, shape=day#, group = "BirdID"#, size=tolerance_cat_num
             )) +
    geom_point() +
    geom_line() +
    # ggbeeswarm::geom_quasirandom(alpha=1) +
...
y_tolerance$counts %>%
  ...
  #mutate(diff_21 = (day21) - (day0),
         # diff_14 = (day14) - (day0)) %>%
  pivot_longer(c("day14", "day21", "day0"), names_to = "day", values_to = "counts") %>% #View()
  filter(day=="day14", MG.Status == "MG") %>% #View()
  ggplot(aes(x = tolerance_median, y = log2(counts)#, color = condition#, shape=day#, group = "BirdID"#, size=tolerance_cat_num
             )) +
    geom_point() +
    # geom_line() +
    # ggbeeswarm::geom_quasirandom(alpha=1) +
...
y_tolerance$counts %>%
  ...
  ggplot(aes(x = tolerance_median, y = log2(counts), #color = condition#, 
