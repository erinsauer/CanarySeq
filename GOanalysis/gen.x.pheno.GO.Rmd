...
Quick visualizaiton of correlation of these pos and neg_cor_traj genes with traj values.
```{r}
...
genPhen_dat_t.tmp14_traj_DE_FC %>% 
  ...
  ggtitle("NEGATIVE: Correlation between L2FC and eye_score recovery slope MGpost0") +
  labs(x = "slope of eye recovery regression",
       y = "log2FC (day 14 vs day 0)") +
  facet_wrap(~gene_name
             # , scales = "free_y"
             ) +
  ggpubr::stat_cor(label.y = -2.5)+
  theme_minimal()
# 
...
genPhen_dat_t.tmp14_traj_DE_FC %>% 
  ...
  dplyr::select(-gene) %>%
  pivot_longer(!c(bird, Diet, eye_score), names_to = "gene_name", values_to = "fold_change") %>%
  filter(gene_name %in% pos_cor_traj_genes) %>%
  ggplot(aes(x=eye_score, y = fold_change)) +
  geom_point(aes(color = Diet)) +
  geom_smooth(method="lm", color = "black", alpha = 0.7) +
  ggtitle("POSITIVE: Correlation between L2FC and eye_score recovery slope MGpost0") +
  labs(x = "slope of eye recovery regression",
       y = "log2FC (day 14 vs day 0)") +
  facet_wrap(~gene_name
             # , scales = "free_y"
...
In short, it appears that protein related genes most strongly correlate with recovery times, while ABLIM3 has the strongest slope + correlation combination for infection recovery.

Hold up now, interferon is showing up more than I'd expect. 

First, MDY88 has high pos corr, and is an IFN related gene. Then, there is USP18 who shows up in MGpost0 contrast but not here.

LOC103818060 Ig lambda chain C region-like shows a R=.64 but mainly driven by those with slow recovery time
LOC103818269 Ig lambda chain V-1 region-like shows R=0.7, but IS A WITHDRAWN GENE!


# Load in PCR cor.traj data (not made yet)
```{r load}
...
# Create a 3x3 correlation plot with the correlation between max eye score, (maybe just day14 as well?), AUC eyescore, and traj eyescores to see how well they compare
```{r}
library(corrplot)
```
