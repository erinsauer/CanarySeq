...
tbl_tol_results %>%
  ...
  left_join(topTags_protein_inf_edgeR, by = c("gene.z" = "gene", "gene_name" = "gene_name", #"logCPM_inf"="logCPM", 
                                              "description" = "description"),suffix = c("_inf_lipid", "_inf_prot")) %>%
  filter(#FDR_tol < 0.05 & 
           FDR_inf < 0.05 & FDR_diet < 0.05 & FDR_ixd < 0.05 #& 
           & FDR_inf_lipid < 0.05
           ) %>%
  ...
  gt::data_color(columns = c(logFC_inf, logFC_inf_lipid, logFC_inf_prot, logFC_diet, logFC_ixd),
             fn = scales::col_numeric(palette = c("blue","white", "red"), domain = c(-5,5))) %>%
  gt::cols_align(columns = c(logFC_inf, logFC_inf_lipid, logFC_inf_prot, logFC_diet, logFC_ixd), align="center") %>%
  gt::tab_style(style = 
              list(
...
merged_results <- tbl_tol_results %>%
  ...
  left_join(topTags_ixd_edgeR, by = c("gene.z" = "gene", "gene_name" = "gene_name", "logCPM_inf"="logCPM", "description" = "description"),suffix = c("_logLoad", "_ixd")) %>%
...
fig.s4 <- merged_results %>%
  upset(., c("DE_inf",#"DE_inf_prot", "DE_inf_lipid", 
             "DE_diet", "DE_ixd", "DE_ES"#, "DE_logLoad"
             ), name="Comparison",
...
nudge_x = -0.4,force=0.1,
        ...
        'Interaction\n(Protein-Lipid)'=(
            # note that aes(x=intersection) is supplied by default and can be skipped
            ggplot(mapping=aes(y=logFC_ixd,color=-log10(FDR_ixd), label=goi#_ixd
                               ))
            # checkout ggbeeswarm::geom_quasirandom for better results!
            + ggbeeswarm::geom_quasirandom(alpha=0.5, size=pt_size,
                na.rm=TRUE) + ggrepel::geom_text_repel(#min.segment.length = 0.1, 
...
## Figure S7: PCA plot
```{r}
PCA <- plotMDS(y_ixd, gene.selection="common",
