...
matches <- which(toupper(names(canary_geneID2GO_BM)) %in% MGLpost0$...1)
...
matches_rnaIndex <- which(MGLpost0$...1 %in% toupper(names(canary_geneID2GO_BM)))
...
noMatch_edgeR.BMwiki <- MGLpost0$...1[-matches_rnaIndex]
...
DEgenes <- filter(MGLpost0, FDR < 0.05 & abs(logFC) > 1) %>% pull(`...1`)
DEgenes <- filter(MGLpost0, abs(logFC) > 0.4) %>% pull(`...1`)
DEgenes <- filter(MGLpost0, -(logFC) > 0.4 & PValue < 0.05) %>% pull(`...1`)
DEgenes <- filter(MGLpost0, (logFC) > 0.585) %>% pull(`...1`)
DEgenes <- filter(MGLpost0, FDR < 0.05 & logFC < -0.585) %>% pull(`...1`)

DEgenes <- filter(MGLpost0, logFC > 0.585 & PValue < 0.1) %>% pull(`...1`)
...
length(DEgenes[-DEgeneswithGO]) #count now that don't have GO terms anymore
```
